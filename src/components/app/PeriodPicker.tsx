...
export function PeriodPicker({value,onChange,min='2000-01',max='2100-12',label='Hisobot davri'}:any){
 ...
 return <div className="period-picker" role="group" aria-label={label}>
  <Button variant="ghost" size="icon" className="period-step" aria-label="Oldingi oy" disabled={!allowed(previous)} onClick={()=>jump(previous)}><Icon name="arrow" className="flip" size={16}/></Button>
  ...
  <Button variant="ghost" size="icon" className="period-step" aria-label="Keyingi oy" disabled={!allowed(next)} onClick={()=>jump(next)}><Icon name="arrow" size={16}/></Button>
 </div>;
}
