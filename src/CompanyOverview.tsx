      ...
      <div className="flex flex-col gap-5 p-5 sm:p-7">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold tracking-[0.14em] text-primary uppercase">{formatPeriod(period)} · hisobot davri</p>
        ...
        {[
          ...
          <li key={item.key} className="flex-1">
            <button type="button" onClick={() => onFilter(item.key)} aria-label={`${item.label}: ${item.value} ta hujjat. Ro‘yxatni ochish`}
              ...
              <span className="text-[1.75rem] leading-none font-semibold tracking-tight tabular-nums">{item.value}</span>
