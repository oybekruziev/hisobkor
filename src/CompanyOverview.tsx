...
export function CompanyOverview({company,docs,period,onNavigate,onOpen,onUpload}:any){
 ...
 return <div className="company-overview">
  ...
  {recent.length?<ul>{recent.map((d:any)=><li key={d.id}><Icon name="document"/><Button variant="ghost" className="overview-document-name" onClick={()=>onOpen(d)}>{documentName(d)}</Button><Badge variant="outline" className="badge" data-status={d.status}>{statuses[d.status]}</Badge><span>{d.date||'—'}</span></li>)}</ul>:<div className="overview-empty"><Icon name="folder" size={28}/><p>Bu davrda hali hujjat yo‘q.</p><Button variant="outline" onClick={onUpload}><Icon name="upload"/>Hujjat yuklash</Button></div>}
  </Card>
