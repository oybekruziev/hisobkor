...
function App({saved,session,logout}:any){
 ...
 if(!profile)return <div className="isolate flex min-h-svh w-full flex-col bg-background antialiased"><div className="grid flex-1 lg:grid-cols-[minmax(0,5fr)_minmax(0,6fr)]">
  <aside className="relative flex flex-col justify-between gap-10 overflow-hidden bg-ink px-6 py-6 text-ink-foreground sm:px-10 lg:py-10">
   <span aria-hidden="true" className="pointer-events-none absolute -bottom-32 -left-24 size-[28rem] rounded-full bg-ink-primary/10 blur-3xl"/>
   <a href="#dashboard" aria-label="Hisobkor.uz" className="relative flex w-fit items-center gap-2.5"><img src="/brand-h.png" width="28" height="28" alt="" className="size-7 shrink-0 object-contain"/><span className="text-lg font-semibold tracking-tight">hisobkor<span className="font-normal text-ink-muted">.uz</span></span></a>
   <div className="relative flex max-w-md flex-col gap-8 max-lg:hidden">
    <div className="flex flex-col gap-3"><p className="text-sm font-medium text-ink-primary">Ishni boshlash</p><h1 className="text-[2.25rem] leading-tight font-semibold tracking-tight text-balance text-white">Barcha kompaniyalaringiz. Bitta tartibli ish joyi.</h1><p className="text-pretty text-white/70">Hujjatlarni yig‘ing, tekshiring va har bir kompaniya bo‘yicha tarixni saqlang.</p></div>
    <ol role="list" className="flex flex-col gap-5">{[['O‘zingizni tanishtiring','Buxgalter ma’lumotlarini kiriting'],['Kompaniya qo‘shing','Xizmat ko‘rsatadigan tashkilotingiz'],['Hujjatlar bilan ishlang','Yuklang, tekshiring va tarixni kuzating']].map(([title,text],i)=><li key={title} className="flex gap-3.5" aria-current={i===0?'step':undefined}><span className={cn('flex size-8 shrink-0 items-center justify-center rounded-full text-sm font-semibold tabular-nums',i===0?'bg-ink-primary text-ink-primary-foreground':'border border-ink-border text-ink-muted')}>{i+1}</span><div><p className={cn('text-sm font-medium',i===0?'text-white':'text-ink-foreground')}>{title}</p><p className="text-sm text-ink-muted">{text}</p></div></li>)}</ol>
   </div>
   <p className="relative flex items-start gap-2 text-sm text-ink-muted max-lg:hidden"><Icon name="folder" size={16} className="mt-0.5 shrink-0"/>Hujjatlar har bir kompaniyada alohida saqlanadi.</p>
  </aside>
    ...
    <SidebarGroup className="min-h-0 flex-1">
     ...
     <SidebarGroupContent className="flex min-h-0 flex-col gap-2">
      ...
      <SidebarMenu aria-label="Kompaniyalar ro‘yxati">{(company?.isDemo?[...mine,company]:mine).filter(c=>`${c.name} ${c.stir||''}`.toLowerCase().includes(companyQuery.toLowerCase())).map(c=>{const n=reviewCount(c.id);const active=company?.id===c.id;return <SidebarMenuItem key={c.id}><SidebarMenuButton size="lg" isActive={active} aria-current={active?'page':undefined} onClick={()=>go(`company/${c.id}/overview`)} className="h-11 data-[active=true]:bg-sidebar-accent data-[active=true]:shadow-xs data-[active=true]:ring-1 data-[active=true]:ring-sidebar-border">
        ...
        {n>0&&<span className="rounded-full bg-amber-100 px-1.5 text-xs font-semibold text-amber-900 tabular-nums" aria-label={`${n} ta hujjat tekshiruvni kutmoqda`}>{n}</span>}
