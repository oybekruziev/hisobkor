    ...
    <Card>
     <CardHeader>
      ...
      <CardAction className="flex items-center gap-2">
       {tab==='documents'&&<Button variant="outline" size="sm" onClick={()=>open({type:'request'})}><Icon name="plus"/><span className="max-sm:sr-only">Hujjat so‘rash</span></Button>}
...
<Modal open={modal?.type==='request'} onClose={()=>setModal(null)} title="Hujjat so‘rash" description={`${company?.name||''} · ${month(period)}`}><form onSubmit={requestDocument} noValidate><FieldGroup><Field data-invalid={error?true:undefined}><FieldLabel htmlFor="request-title">Qanday hujjat kerak?</FieldLabel><Input id="request-title" required name="title" maxLength={120} placeholder="Masalan, sentabr oyi bank ko‘chirmasi" aria-invalid={error?true:undefined}/><FieldDescription>Talab ro‘yxatda “Kutilmoqda” holatida turadi va davr yopilishiga to‘sqinlik qiladi.</FieldDescription>{error&&<FieldError>{error}</FieldError>}</Field><Alert><Icon name="help"/><AlertDescription>Talab kompaniya ro‘yxatiga qo‘shiladi. Hozircha mijozga avtomatik xabar yuborilmaydi.</AlertDescription></Alert></FieldGroup><DialogFooter className="mt-6"><Button variant="outline" type="button" onClick={()=>setModal(null)}>Bekor qilish</Button><Button type="submit">Talab qo‘shish</Button></DialogFooter></form></Modal>
...
<Modal open={modal?.type==='document'} onClose={()=>setModal(null)} title={activeDoc?documentName(activeDoc):'Hujjat'} description={company?.name} wide>{activeDoc&&<div className="grid items-start gap-6 lg:grid-cols-[7fr_5fr]">
 ...
 <section className="flex min-w-0 flex-col gap-4" aria-label="Hujjat ma’lumotlari va qaror">
  ...
  {activeDoc.status==='review_required'&&<Card className="gap-4 bg-muted/40 py-5 shadow-none"><CardHeader className="px-5"><CardTitle>Qaroringiz</CardTitle><CardDescription>Hujjatni ko‘rib chiqing va bitta qarorni tanlang. Qaror kompaniya tarixida ismingiz bilan saqlanadi.</CardDescription></CardHeader><CardContent className="flex flex-col gap-4 px-5">
   <Button className="w-full" onClick={()=>decide('accepted')}><Icon name="check"/>Qabul qilish</Button>
   <div className="flex items-center gap-3 text-xs text-muted-foreground"><Separator className="flex-1"/>yoki<Separator className="flex-1"/></div>
   <form onSubmit={e=>{e.preventDefault();decide('correction_requested',String(new FormData(e.currentTarget).get('reason')||''))}}><FieldGroup><Field data-invalid={error?true:undefined}><FieldLabel htmlFor="decision-reason">Tuzatish sababi</FieldLabel><Textarea id="decision-reason" name="reason" rows={3} className="bg-background" placeholder="Masalan, sentabr oyi o‘rniga avgust oyi hujjati yuklangan" aria-invalid={error?true:undefined} aria-describedby="decision-reason-hint"/>{error?<FieldError>{error}</FieldError>:<FieldDescription id="decision-reason-hint">Sabab majburiy: mijoz nimani tuzatishi kerakligini yozing.</FieldDescription>}</Field><Button type="submit" variant="outline" className="w-full"><Icon name="alert"/>Tuzatishga qaytarish</Button></FieldGroup></form>
  </CardContent></Card>}
