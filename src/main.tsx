...
function Empty({title,text,action,icon='folder',pose}:any){return <EmptyRoot><EmptyHeader>{pose?<Character pose={pose} width={150} className="mb-2 max-sm:w-28!"/>:<EmptyMedia variant="icon"><Icon name={icon}/></EmptyMedia>}<EmptyTitle>{title}</EmptyTitle><EmptyDescription>{text}</EmptyDescription></EmptyHeader>{action&&<EmptyContent>{action}</EmptyContent>}</EmptyRoot>}
...
function App({saved,session,logout}:any){
 ...
 const ai=useAI({docs,setDocs,companies,onEvent:event,auto:aiAuto,setAuto:setAiAuto,enabled:!!profile,canAnalyze:()=>saveStatus==='saved'&&!pendingSave.current&&!hasPendingWrites()});
 ...
 if(!profile)return <div className="isolate flex min-h-svh w-full flex-col bg-background antialiased"><div className="grid flex-1 lg:grid-cols-[minmax(0,5fr)_minmax(0,6fr)]">
  <aside className="relative flex flex-col justify-between gap-8 overflow-hidden bg-[linear-gradient(160deg,#EAF0FB_0%,#F7F9FC_70%)] px-6 py-6 text-foreground sm:px-10 lg:border-r lg:py-10">
   <a href="#dashboard" aria-label="Hisobkor.uz" className="relative w-fit"><Wordmark size={32}/></a>
   <div className="relative flex max-w-md flex-col gap-7 max-lg:hidden">
    <Character pose="welcome" width={220} eager className="-ml-2"/>
    <div className="flex flex-col gap-3"><p className="text-xs font-semibold tracking-[0.14em] text-primary uppercase">Xush kelibsiz</p><h1 className="text-[2.125rem] leading-tight font-semibold tracking-[-0.025em] text-balance">Bir nechta kompaniya. Bitta aniq ish jarayoni.</h1><p className="text-pretty text-muted-foreground">Hujjatlarni yig‘ing, tekshiring va har bir kompaniya bo‘yicha qaror tarixini saqlang.</p></div>
    <ol role="list" className="flex flex-col gap-4">{[['O‘zingizni tanishtiring','Buxgalter ma’lumotlarini kiriting'],['Kompaniya qo‘shing','Xizmat ko‘rsatadigan tashkilotingiz'],['Hujjatlar bilan ishlang','Yuklang, tekshiring va tarixni kuzating']].map(([title,text],i)=><li key={title} className="flex gap-3.5" aria-current={i===0?'step':undefined}><span className={cn('flex size-8 shrink-0 items-center justify-center rounded-full text-sm font-semibold tabular-nums',i===0?'bg-primary text-primary-foreground':'border bg-card text-muted-foreground')}>{i+1}</span><div><p className="text-sm font-medium">{title}</p><p className="text-sm text-muted-foreground">{text}</p></div></li>)}</ol>
   </div>
   <p className="relative flex items-start gap-2 text-sm text-muted-foreground max-lg:hidden"><Icon name="shield" size={16} className="mt-0.5 shrink-0"/>Har bir kompaniyaning hujjatlari alohida saqlanadi.</p>
  </aside>
 ...
 const historyTone:any={check:'border-success/25 bg-success/10 text-success',request:'border-warning/40 bg-warning/10 text-warning-ink',alert:'border-danger/25 bg-danger/10 text-danger',spark:'border-primary/20 bg-accent text-accent-foreground'};
 ...
 return <>
  ...
  <Sidebar variant="inset" collapsible="offcanvas"><div role="navigation" aria-label="Asosiy menyu" className="contents">
   <SidebarHeader className="gap-4 px-3 pt-5 pb-2">
    <a href="#dashboard" aria-label="Hisobkor.uz bosh sahifasi" className="flex items-center gap-2.5 rounded-lg px-1.5 py-1 outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring"><Wordmark size={30} muted="text-sidebar-muted"/></a>
   ...
   <SidebarContent className="gap-0">
    <SidebarGroup><SidebarGroupContent><SidebarMenu aria-label="Asosiy">
     <SidebarMenuItem><SidebarMenuButton isActive={!company&&!isMsfo} aria-current={company||isMsfo?undefined:'page'} onClick={()=>go('dashboard')}><Icon name="dashboard"/><span>Bosh sahifa</span>{attentionTotal>0&&<span className="ml-auto rounded-full bg-warning/12 px-1.5 text-xs font-semibold text-warning tabular-nums">{attentionTotal}</span>}</SidebarMenuButton></SidebarMenuItem>
     <SidebarMenuItem><SidebarMenuButton isActive={isMsfo} aria-current={isMsfo?'page':undefined} onClick={()=>go('msfo')}><Icon name="msfo"/><span>MHXS hujjatlari</span></SidebarMenuButton></SidebarMenuItem>
    ...
    <SidebarGroup className="min-h-0 flex-1">
     ...
     <SidebarGroupContent className="flex min-h-0 flex-col gap-2">
      ...
      <SidebarMenu aria-label="Kompaniyalar ro‘yxati">{(company?.isDemo?[...mine,company]:mine).filter(c=>`${c.name} ${c.stir||''}`.toLowerCase().includes(companyQuery.toLowerCase())).map(c=>{const n=reviewCount(c.id);const active=company?.id===c.id;return <SidebarMenuItem key={c.id}><SidebarMenuButton size="lg" isActive={active} aria-current={active?'page':undefined} onClick={()=>go(`company/${c.id}/overview`)} className="h-11 data-[active=true]:bg-sidebar-accent data-[active=true]:shadow-xs data-[active=true]:ring-1 data-[active=true]:ring-sidebar-border">
        ...
        {n>0&&<span className="rounded-full bg-warning/12 px-1.5 text-xs font-semibold text-warning tabular-nums" aria-label={`${n} ta hujjat tekshiruvni kutmoqda`}>{n}</span>}
   ...
   <SidebarFooter className="p-3"><div className="rounded-xl border border-sidebar-border bg-sidebar-accent shadow-[0_1px_2px_rgb(21_35_59/0.05)]">
     ...
     <Dropdown.DropdownMenu modal={false}>
      <Dropdown.DropdownMenuTrigger asChild>
       <SidebarMenuButton size="lg" className="data-[state=open]:bg-sidebar-accent" aria-label="Hisob menyusi">
        <Avatar className="size-8 rounded-full"><AvatarFallback className="rounded-full bg-sidebar-primary text-xs font-semibold text-sidebar-primary-foreground">{initials(profile.fullName)}</AvatarFallback></Avatar>
  ...
  <SidebarInset className="min-w-0 bg-[radial-gradient(900px_320px_at_15%_-5%,rgb(214_228_250/0.55),transparent_70%)]">
   <header className="sticky top-0 z-20 flex h-14 shrink-0 items-center gap-2 border-b border-border/70 bg-background/80 px-4 backdrop-blur-md lg:rounded-t-xl lg:px-8">
    ...
    <Breadcrumb className="min-w-0" aria-label="Sahifa yo‘li"><BreadcrumbList className="flex-nowrap">
     ...
     {isMsfo&&<><BreadcrumbSeparator className="max-sm:hidden"/><BreadcrumbItem className="min-w-0">{msfoItem?<BreadcrumbLink href="#msfo">MHXS hujjatlari</BreadcrumbLink>:<BreadcrumbPage>MHXS hujjatlari</BreadcrumbPage>}</BreadcrumbItem>{msfoItem&&<><BreadcrumbSeparator className="max-md:hidden"/><BreadcrumbItem className="min-w-0 max-md:hidden"><BreadcrumbPage className="truncate">{msfoItem.title}</BreadcrumbPage></BreadcrumbItem></>}</>}
...
{isMsfo?<MsfoPage onAssign={(item:any,cid:string)=>{const c=companies.find(x=>x.id===cid);if(c)event('MHXS hujjati kompaniyaga biriktirildi',c,item.title,'company');notify.success('Hujjat kompaniyaga biriktirildi.')}} items={msfo} setItems={setMsfo} companies={companies} route={route} go={go} aiConnected={!!ai.connection.connected} aiLoading={!!ai.connection.loading} aiConsent={aiAuto} setAiConsent={setAiAuto}/>:!company?<>
    ...
    <a href="#msfo" className="group/msfo flex items-center gap-4 rounded-2xl border border-dashed border-primary/30 bg-accent/50 p-4 outline-none hover:border-primary/60 hover:bg-accent focus-visible:ring-[3px] focus-visible:ring-ring/50 sm:px-6">
     ...
     <span className="grid min-w-0 flex-1 gap-0.5"><span className="flex flex-wrap items-center gap-2 font-medium">MHXS redaktori<span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">Yangi</span></span><span className="text-sm text-pretty text-muted-foreground">PDF yoki Word hisobotni AI yordamida MHXS (MSFO / IFRS) shakliga o‘tkazing va .docx qilib oling.{msfo.length?` · ${msfo.length} ta hujjat`:''}</span></span>
    ...
    <header className="flex flex-col gap-5">
     <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div className="flex min-w-0 items-center gap-4">
       <InitialsTile name={company.name} size="lg" className="size-14 rounded-2xl bg-gradient-to-br from-[#2A5BDB] to-[#1239A7] text-lg text-primary-foreground shadow-[inset_0_1px_0_rgb(255_255_255/0.2),0_8px_18px_-10px_rgb(18_57_167/0.45)]"/>
      ...
      {visible.length?<div className="overflow-hidden rounded-xl border"><Table>
       ...
       <TableBody>{pageDocs.map(d=><TableRow key={d.id}>
        <TableCell className="whitespace-normal"><button type="button" className="flex w-full min-w-36 items-center gap-3 rounded-md text-left outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50" onClick={()=>showDocument(d)}>
         {d.status==='missing'?<span aria-hidden="true" className="flex size-9 shrink-0 items-center justify-center rounded-lg border border-dashed border-warning/40 bg-warning/10 text-warning-ink"><Icon name="clock" size={16}/></span>:<FileTile fileName={d.fileName}/>}
...
{modal?.type==='upload'&&<UploadForm onSubmit={upload} busy={busy} error={error} archive={tab==='archive'} target={modal.target} pick={!!modal.pick} companies={mine} aiConnected={!!ai.connection.connected} aiAuto={aiAuto} setAiAuto={setAiAuto}/>}</Modal>
...
<Modal open={modal?.type==='document'} onClose={()=>setModal(null)} title={activeDoc?documentName(activeDoc):'Hujjat'} description={company?.name} wide>{activeDoc&&<div className="grid items-start gap-6 lg:grid-cols-[7fr_5fr]">
 ...
 <section className="flex min-w-0 flex-col gap-4" aria-label="Hujjat ma’lumotlari va qaror">
  ...
  {activeDoc.reason&&<Alert className="border-warning/40 bg-warning/10 text-warning-ink"><Icon name="alert"/><AlertTitle>Tuzatish sababi</AlertTitle><AlertDescription className="text-warning">{activeDoc.reason}</AlertDescription></Alert>}
...
function UploadForm({onSubmit,busy,error,archive,target,companies,pick,aiConnected,aiAuto,setAiAuto}:any){const [files,setFiles]=useState<File[]>([]);const [over,setOver]=useState(false);return <form onSubmit={e=>onSubmit(e,files)} noValidate><FieldGroup>
 ...
 {aiConnected&&<AiConsent checked={aiAuto} onCheckedChange={setAiAuto}/>}
 {error&&<FieldError>{error}</FieldError>}
</FieldGroup><DialogFooter className="mt-6 sm:items-center"><span className="text-sm text-pretty text-muted-foreground sm:mr-auto">Hujjat avtomatik qabul qilinmaydi — qarorni siz berasiz.</span><Button type="submit" disabled={busy}>{busy?<Spinner/>:<Icon name="upload"/>}{busy?'Saqlanmoqda…':'Yuklash'}</Button></DialogFooter></form>}
