...
export function AIBadge({doc}:any){
 ...
 return <span className="ai-badge"><Icon name="spark" size={14}/>{text}</span>;
}
export function AIBar({ai,docs}:any){
 ...
 return <section className="ai-bar" aria-label="Avtomatik hujjat tekshiruvi"><div className="ai-bar-title"><span className="ai-mark"><Icon name="spark"/></span><div><strong>Avtomatik tekshiruv</strong><p>{ai.connection.loading?'Holat tekshirilmoqda…':ai.connection.connected?pending?`${pending} ta hujjat tekshirilmoqda · ${done} ta tayyor`:`${done} / ${files.length} hujjat tekshirilgan`:'Xizmat hozir mavjud emas; hujjatlarni qo‘lda tekshiring'}</p></div></div>{ai.connection.connected&&<div className="ai-bar-actions"><Button variant="outline" disabled={!ai.ready||ai.running||!files.some((d:any)=>!validAnalysis(d)&&!['queued','processing'].includes(d.ai?.status))} onClick={()=>ai.check(files.filter((d:any)=>!validAnalysis(d)))}><Icon name="spark"/>{ai.running?'Tekshirilmoqda…':'Tekshirishni boshlash'}</Button></div>}{ai.notice&&<p className="ai-notice" role="status">{ai.notice}</p>}</section>;
}
